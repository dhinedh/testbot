  ...
  return (
    <div className="flex-1 bg-[#efeae2] rounded-lg flex flex-col relative shadow-sm h-[500px] md:h-auto overflow-hidden">
      ...
      {(() => {
        ...
        const displayId = isIG ? `Instagram ID: ${contact.phone.replace('ig:', '')}` : isFB ? `Messenger ID: ${contact.phone.replace('fb:', '')}` : `+${contact.phone}`;
        const channelTag = isIG ? '📸 Instagram Direct' : isFB ? '💬 Facebook Messenger' : '🟢 WhatsApp Business';

        return (
          <div className="bg-bg-light px-5 py-4 border-b border-border-color flex justify-between items-center z-10">
            <div>
              <div className="flex items-center gap-2">
                <h2 className="font-semibold text-lg text-text-dark">{contact.name || displayId}</h2>
                <span className="text-xs bg-white text-gray-700 px-2 py-0.5 rounded border border-gray-200 font-medium">{channelTag}</span>
              </div>
              <p className="text-sm text-text-muted">{displayId}</p>
            </div>
